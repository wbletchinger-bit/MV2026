...
function createSupabaseClientSafely(url, anonKey) {
  ...
  if (!isLikelySupabaseUrl(url)) {
    return {
      client: null,
      configError:
        'The Supabase URL should look like https://your-project-ref.supabase.co',
    };
  }
  if (!String(anonKey).trim().startsWith('eyJ')) {
    return {
      client: null,
      configError:
        'The Supabase key should usually start with eyJ. Make sure you copied the anon/publishable key, not the secret key label.',
    };
  }
  try {
    return {
    ...
    return {
      client: null,
      configError: error.message || 'Could not create the Supabase client.',
    };
  }
}

const supabaseConfig = createSupabaseClientSafely(
  SUPABASE_URL,
  SUPABASE_ANON_KEY
);
...
const tripData = {
  houses: [
    {
      ...
      beds: 6,
      address:
        '10 Huckleberry Hill Rd, Edgartown, MA, 02539 United States of America',
    ...
    },
    {
      ...
      beds: 5,
      address:
        '167 Pennywise Path, Edgartown, MA, 02539 United States of America',
  ...
  bedAssignments: [
    ...
    {
      house: 'The Big House',
      bed: '1st Floor King/Bath',
      names: 'Erita, Kyle',
      week: 'Full',
      image: 'H1-K2.jpg',
    },
    {
      house: "The Kid's Hangout",
      bed: 'Double Bed',
      names: 'Carl, Aye',
      week: 'Full',
      image: 'H2-Double.jpg',
    ...
    },
    {
      house: "The Kid's Hangout",
      bed: 'King',
      names: 'Adrian, Martine, Valentina, Genevieve',
      week: 'Full',
      image: 'H2-King.jpg',
    },
  ],
  ...
  links: [
    { title: 'Join the VRBO Trip', url: 'https://t.vrbo.io/X0SBiGEcmVb' },
    {
      ...
      url: 'https://www.zola.com/wedding/jessicasauberandaidanbarrett',
    },
    {
      title: 'Points of Interest',
      url: 'https://maps.app.goo.gl/8pfeJr2ZDPos4G829',
    },
...
const moreItems = navItems.filter(
  (item) => !['home', 'houses', 'meals'].includes(item.id)
);
const noNotesMeals = new Set([
  'Eat Out',
  'On your own',
  'Rehearsal Dinner',
  'Wedding',
...
function createMealState() {
  return tripData.meals.map((meal) => ({
    ...meal,
    cooks: [...meal.cooks],
    notes: '',
    expanded: false,
...
const starterEvents = [
  {
    ...
    notes:
      'Drop bags, claim beds, locate sunscreen, pretend we packed efficiently.',
    ...
    new Promise((_, reject) => {
      window.setTimeout(
        () =>
          reject(
            new Error(
              `${label} timed out. Check the Supabase URL, key, table, and policies.`
            )
          ),
        ms
      );
...
function linkIcon(title, category) {
  ...
  if (category === 'Map' || lower.includes('interest') || lower.includes('map'))
    return '📍';
...
function runSelfTests() {
  ...
  console.assert(
    ...
    'parseTime should handle PM times'
  );
  console.assert(
    normalizeUrl('example.com') === 'https://example.com',
    'normalizeUrl should add https'
  );
  console.assert(
    inferLinkCategory('Wedding Website') === 'Wedding',
    'Wedding links should be categorized'
  );
  console.assert(
    sharedKeys.events === 'calendar_events',
    'Shared state keys should be stable'
  );
  console.assert(
    typeof withTimeout === 'function',
    'withTimeout should exist for Supabase diagnostics'
  );
}
...
function GlassCard({ children, className = '' }) {
  return (
    <div
      className={`rounded-[1.6rem] border border-white/50 bg-white/45 shadow-xl shadow-sky-900/5 backdrop-blur-xl ${className}`}
    >
      {children}
    </div>
  );
...
function ConfirmButton({ children, message, onConfirm, className = '' }) {
  return (
    <button
      type="button"
      onClick={() => window.confirm(message) && onConfirm()}
      className={className}
    >
      {children}
    </button>
...
function ImageWithFallback({ src, alt, className = '' }) {
  ...
  if (failed) {
    return (
      <div
        className={`grid place-items-center bg-gradient-to-br from-sky-100 via-yellow-50 to-pink-100 text-sm text-slate-500 ${className}`}
      >
        {alt}
      </div>
    );
...
function NameSelect({
  ...
  return (
    ...
    >
      <option value="">
        {selectedGuest ? `Use ${selectedGuest}` : placeholder}
      </option>
    ...
    <header className="sticky top-0 z-20 border-b border-white/40 bg-white/45 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        <button
          ...
          className="text-left"
        >
          <div className="text-xs uppercase tracking-[0.28em] text-sky-700/80">
            MV 2026
          </div>
    ...
    <>
      ...
      <nav className="fixed bottom-3 left-3 right-3 z-30 grid grid-cols-4 rounded-[1.7rem] border border-white/60 bg-white/65 p-2 shadow-2xl shadow-slate-900/10 backdrop-blur-2xl md:hidden">
        {primaryNavItems.map((item) => {
          const isActive =
            item.id === 'more'
              ? moreItems.some((more) => more.id === activePage)
              : activePage === item.id;
          return (
            <button
              key={item.id}
              ...
              className={`rounded-2xl px-1 py-2 text-center text-[11px] ${
...
function HomePage({ setActivePage, events }) {
  ...
  const todaysEvents = [...events]
    .filter((event) => event.day === today.day)
    .sort(sortByTime);
    ...
    <div className="space-y-5">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          >
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-700 sm:text-lg">
              Houses, beds, meals, packing lists, links, and the small amount of
              structure required to keep vacation feeling like vacation.
            </p>
        ...
        <GlassCard className="p-5">
          ...
          <div className="space-y-3">
            {todaysEvents.length ? (
                ...
                >
                  ...
                  {event.requiresSignup && (
                    <div className="mt-2 text-xs font-semibold text-yellow-900">
                      ...
                      {event.signupLimit ? `/${event.signupLimit}` : ''} signed
                      up
                    </div>
                  )}
                </button>
              ...
              <div className="rounded-3xl bg-white/45 p-4 text-sm text-slate-600">
                No events today. Dangerous levels of freedom.
              </div>
            )}
          </div>
        </GlassCard>
      ...
      <GlassCard className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Live weather will plug in here during deployment.
            </p>
          </div>
        ...
        <div className="mt-5 rounded-[1.5rem] bg-gradient-to-br from-sky-100/80 to-yellow-100/70 p-5">
          ...
          <div className="mt-1 text-sm text-slate-600">
            Placeholder: breezy, beachy, suspiciously perfect
          </div>
        </div>
      </GlassCard>
    </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {tripData.houses.map((house) => {
        const beds = tripData.bedAssignments.filter(
          (bed) => bed.house === house.name
        );
        ...
        return (
          <GlassCard key={house.name} className="overflow-hidden">
            <button
              ...
              onClick={() => setOpenHouse(isOpen ? '' : house.name)}
            >
              <div className="grid gap-0 md:grid-cols-[0.95fr_1.05fr]">
                <ImageWithFallback
                  src={assetPath(house.image)}
                  alt={house.name}
                ...
                <div className="p-5 sm:p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="rounded-full bg-white/70 px-3 py-1 text-sm font-semibold text-slate-700">
                      {isOpen ? '−' : '+'}
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex gap-3 text-sm">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="rounded-full bg-white/70 px-4 py-2 font-semibold text-slate-800"
                    >
                      VRBO
                    </a>
                  </div>
                ...
                >
                  <div className="grid gap-3 border-t border-white/50 p-4 sm:grid-cols-2 lg:grid-cols-3">
                    {beds.map((bed) => (
                      <div
                        key={bed.bed}
                        className="overflow-hidden rounded-[1.35rem] bg-white/50 shadow-sm"
                      >
                        <ImageWithFallback
                          src={assetPath(bed.image)}
                          alt={bed.bed}
...
function PackingPage({
  ...
  const claimedCount = groupItems.filter((item) => item.person?.trim()).length;
  ...
  function addGroupItem(event) {
    ...
    setGroupItems((current) => [
      ...current,
      { id: `${Date.now()}-${trimmed}`, item: trimmed, person: '' },
    ]);
    setNewItem('');
  }

  function updateGroupPerson(index, person) {
    setGroupItems((current) =>
      current.map((entry, i) => (i === index ? { ...entry, person } : entry))
    );
  }
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-3 max-w-2xl leading-7 text-slate-700">
          Personal packing stays on your device. Group items sync across guests
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <span className="ml-1 opacity-70">
              {packedCount}/{tripData.personalPacking.length}
            </span>
          </button>
          <button
            type="button"
          ...
          >
            Group{' '}
            <span className="ml-1 opacity-70">
              {claimedCount}/{groupItems.length}
            </span>
          </button>
        </div>
      </GlassCard>
            ...
            <GlassCard className="p-4 sm:p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    Tap an item when it’s packed.
                  </p>
                </div>
                <ConfirmButton
                  message="Reset your personal packing checklist?"
                  onConfirm={() => setCheckedPersonal({})}
                  className="rounded-full bg-white/55 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white/80"
                >
                  Reset
                </ConfirmButton>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 rounded-2xl p-3 text-left transition ${
                        checked
                          ? 'bg-sky-100/75 text-slate-500'
                          : 'bg-white/45 text-slate-800 hover:bg-white/75'
                      }`}
                    >
                      <span
          ...
          >
            <GlassCard className="p-4 sm:p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Claim an item with a guest name. Admins can remove items.
                </p>
              </div>

              <form
                onSubmit={addGroupItem}
                className="mb-4 flex gap-2 rounded-[1.5rem] bg-white/40 p-2"
              >
                <input
                  value={newItem}
                  onChange={(event) => setNewItem(event.target.value)}
                  placeholder="Add a group item"
                  className="min-w-0 flex-1 rounded-full bg-white/70 px-4 py-3 text-sm outline-none placeholder:text-slate-400"
                />
              ...
              <div className="space-y-2">
                {groupItems.map((entry, index) => (
                  <div
                    key={entry.id || `${entry.item}-${index}`}
                    className="grid gap-3 rounded-[1.35rem] bg-white/45 p-3 sm:grid-cols-[1fr_220px_auto] sm:items-center"
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`h-3 w-3 rounded-full ${
                          entry.person?.trim() ? 'bg-sky-400' : 'bg-yellow-300'
                        }`}
                      />
                      <div>
                    ...
                    {isAdmin && (
                      <ConfirmButton
                        ...
                        onConfirm={() => deleteGroupItem(index)}
                        className="rounded-full bg-white/70 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white"
                      >
                        Delete
                      </ConfirmButton>
                    )}
...
function MealsPage({ selectedGuest, meals, setMeals, isAdmin, sync }) {
  ...
  function updateNotes(mealIndex, notes) {
    setMeals((current) =>
      current.map((meal, index) =>
        index === mealIndex ? { ...meal, notes } : meal
      )
    );
  }
  ...
  function toggleNotes(mealIndex) {
    setMeals((current) =>
      current.map((meal, index) =>
        index === mealIndex ? { ...meal, expanded: !meal.expanded } : meal
      )
    );
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl leading-7 text-slate-700">
            Sign up to cook and add recipe notes. Guest dropdowns keep names
            tidy.
          </p>
          <div className="mt-3">
            <SyncBadge sync={sync} />
        ...
        {isAdmin && (
          <ConfirmButton
            message="Reset all meal signups and notes?"
            onConfirm={() => setMeals(createMealState())}
            className="w-fit rounded-full bg-white/55 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white/80"
          >
            Reset meals
          </ConfirmButton>
        )}
            ...
            <GlassCard key={day} className="overflow-hidden">
              ...
              <div className="grid gap-3 p-4">
                {dayMeals.map((meal) => {
                  ...
                  return (
                    <div
                      key={`${meal.day}-${meal.time}-${meal.meal}`}
                      className="rounded-[1.45rem] bg-white/45 p-4 shadow-sm"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="text-xs uppercase tracking-[0.18em] text-slate-500">
                      ...
                      {meal.cooks.length ? (
                        <div className="mt-4 grid gap-2 sm:grid-cols-3">
                          {meal.cooks.map((cook, cookIndex) => (
                            <label
                              key={`${meal.day}-${meal.time}-${cookIndex}`}
                              className="block"
                            >
                        ...
                        <div className="mt-4 rounded-2xl bg-sky-100/55 px-3 py-2 text-sm text-slate-700">
                          No cook signup needed.
                        </div>
                      )}
                      ...
                      {canHaveNotes && (
                        <div className="mt-4">
                          <button
                            type="button"
                            onClick={() => toggleNotes(mealIndex)}
                            className="rounded-full bg-white/60 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white/85"
                          >
                            {meal.expanded
                              ? 'Hide notes'
                              : 'Add recipe / notes'}
                            ...
                            {meal.expanded && (
                              ...
                              >
                                <textarea
                                  value={meal.notes}
                                  onChange={(event) =>
                                    updateNotes(mealIndex, event.target.value)
                                  }
      ...
      <div className="rounded-[2rem] border border-white/50 bg-white/45 p-5 shadow-xl shadow-sky-900/5 backdrop-blur-xl">
        <div className="grid gap-3">
          ...
          <label className="block">
            ...
            <input
              ...
              onBlur={(event) =>
                updateLink(link.id, { url: normalizeUrl(event.target.value) })
              }
              className="w-full rounded-full bg-white/70 px-4 py-3 text-sm outline-none"
            />
          </label>
          <label className="block">
            ...
            <select
              ...
              onChange={(event) =>
                updateLink(link.id, { category: event.target.value })
              }
              className="w-full rounded-full bg-white/70 px-4 py-3 text-sm outline-none"
            >
        ...
        <div className="mt-4 flex gap-2">
          ...
          <ConfirmButton
            ...
            className="rounded-full bg-white/70 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white"
          >
            Delete
          </ConfirmButton>
        </div>
    ...
    <div className="group rounded-[2rem] border border-white/50 bg-white/45 p-5 shadow-xl shadow-sky-900/5 backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/70 hover:shadow-2xl hover:shadow-sky-900/10">
      <div className="flex items-start justify-between gap-4">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/60 text-2xl transition group-hover:scale-105"
        ...
        <div className="flex gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-full bg-white/55 px-3 py-1 text-xs font-semibold text-slate-600 hover:bg-white"
          >
            Open ↗
          </a>
          {isAdmin && (
            <button
              type="button"
              onClick={() => updateLink(link.id, { editing: true })}
              className="rounded-full bg-white/55 px-3 py-1 text-xs font-semibold text-slate-600 hover:bg-white"
            >
              Edit
            </button>
      ...
      <a href={link.url} target="_blank" rel="noreferrer" className="block">
        <div className="mt-5 flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-sky-100/70 px-2.5 py-1 text-xs font-semibold text-slate-600">
            {link.category || 'Other'}
          </span>
        </div>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-3 max-w-2xl leading-7 text-slate-700">
          Add, edit, and organize useful links in one place.
        </p>
        <div className="mt-3">
          <SyncBadge sync={sync} />
        </div>
      ...
      {isAdmin && (
        <GlassCard className="p-4 sm:p-5">
          ...
          <form onSubmit={addLink} className="mt-4 grid gap-3">
            ...
            <button className="w-fit rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white">
              Add link
            </button>
          </form>
        </GlassCard>
      ...
      <div className="rounded-[1.45rem] bg-white/50 p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-[150px_130px_1fr]">
          <select
            ...
            onChange={(changeEvent) =>
              updateEvent(event.id, { day: changeEvent.target.value })
            }
            className="rounded-full bg-white/75 px-4 py-2.5 text-sm outline-none"
          >
            {dayOptions.map((day) => (
          ...
          <input
            value={event.time}
            onChange={(changeEvent) =>
              updateEvent(event.id, { time: changeEvent.target.value })
            }
            placeholder="Time"
            className="rounded-full bg-white/75 px-4 py-2.5 text-sm outline-none placeholder:text-slate-400"
          />
          <input
            value={event.title}
            onChange={(changeEvent) =>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_180px]">
          <input
            ...
            onChange={(changeEvent) =>
              updateEvent(event.id, { location: changeEvent.target.value })
            }
            placeholder="Location"
            className="rounded-full bg-white/75 px-4 py-2.5 text-sm outline-none placeholder:text-slate-400"
          />
          <input
            value={event.signupLimit}
            disabled={!event.requiresSignup}
            onChange={(changeEvent) =>
              updateEvent(event.id, { signupLimit: changeEvent.target.value })
            }
            placeholder="Signup limit"
            className="rounded-full bg-white/75 px-4 py-2.5 text-sm outline-none placeholder:text-slate-400 disabled:opacity-40"
          />
        </div>
          ...
          <input
            ...
            onChange={(changeEvent) =>
              updateEvent(event.id, {
                ...
                signupLimit: changeEvent.target.checked
                  ? event.signupLimit
                  : '',
        ...
        <textarea
          value={event.notes}
          onChange={(changeEvent) =>
            updateEvent(event.id, { notes: changeEvent.target.value })
          }
          placeholder="Notes"
          className="mt-3 min-h-24 w-full rounded-[1.25rem] bg-white/75 p-4 text-sm leading-6 outline-none placeholder:text-slate-400"
        />

        <div className="mt-3 flex gap-2">
          <button
            type="button"
            onClick={() => updateEvent(event.id, { editing: false })}
            className="rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white"
          >
            Done
          </button>
    ...
    <div className="rounded-[1.45rem] bg-white/45 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        {isAdmin && (
          <button
            type="button"
            onClick={() => updateEvent(event.id, { editing: true })}
            className="rounded-full bg-white/60 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-white"
          >
            Edit
          </button>
        )}
      ...
      {event.notes && (
        <p className="mt-3 rounded-2xl bg-white/45 p-3 text-sm leading-6 text-slate-700">
          {event.notes}
        </p>
      )}
        ...
        <div className="mt-4 rounded-[1.25rem] bg-yellow-100/45 p-3">
              ...
              {event.attendees.map((attendee) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      canRemove && removeAttendee(event.id, attendee)
                    }
                    className="rounded-full bg-white/70 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-white"
                  >
                    {attendee}
                    {canRemove ? ' ×' : ''}
                  </button>
          ...
          >
            <NameSelect
              ...
              selectedGuest={selectedGuest}
              placeholder={isFull ? 'Full' : 'Choose name'}
            />
            <button
              disabled={isFull || !signupValue}
              className="rounded-full bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"
            >
              Sign up
            </button>
          </form>
...
function CalendarPage({ events, setEvents, selectedGuest, isAdmin, sync }) {
  ...
  function addAttendee(eventId, name) {
    ...
    setEvents((current) =>
      current.map((event) => {
        if (event.id !== eventId || event.attendees.includes(trimmed))
          return event;
        const limit = Number(event.signupLimit);
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl leading-7 text-slate-700">
            Add events, edit them later, and choose whether an event needs
            signups.
          </p>
          <div className="mt-3">
            <SyncBadge sync={sync} />
        ...
        {isAdmin && (
          <ConfirmButton
            message="Reset the full calendar?"
            onConfirm={() => setEvents(starterEvents)}
            className="w-fit rounded-full bg-white/55 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-white/80"
          >
            Reset calendar
          </ConfirmButton>
        ...
        <GlassCard className="p-4 sm:p-5">
          ...
          <form onSubmit={addEvent} className="mt-4 grid gap-3">
            <div className="grid gap-3 sm:grid-cols-[180px_140px_1fr]">
              ...
              <label className="block">
                ...
                <input
                  ...
                  placeholder="Beach, ferry run, grocery mission..."
                  className="w-full rounded-full bg-white/70 px-4 py-3 text-sm outline-none placeholder:text-slate-400"
                />
              </label>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_180px]">
              ...
              />
              <input
                value={eventForm.signupLimit}
                disabled={!eventForm.requiresSignup}
                onChange={(event) =>
                  setEventForm((current) => ({
                    ...current,
                    signupLimit: event.target.value,
                  }))
                }
                placeholder="Signup limit"
                className="w-full rounded-full bg-white/70 px-4 py-3 text-sm outline-none placeholder:text-slate-400 disabled:opacity-40"
              />
            </div>
            ...
            <label className="flex w-fit items-center gap-3 rounded-full bg-white/45 px-4 py-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={eventForm.requiresSignup}
                onChange={(event) =>
                  setEventForm((current) => ({
                    ...current,
                    requiresSignup: event.target.checked,
                    signupLimit: event.target.checked
                      ? current.signupLimit
                      : '',
                  }))
                }
            ...
            <textarea
              ...
              className="min-h-24 w-full rounded-[1.25rem] bg-white/70 p-4 text-sm leading-6 outline-none placeholder:text-slate-400"
            />
            <button className="w-fit rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white">
              Add event
            </button>
            ...
            <GlassCard key={day} className="overflow-hidden">
              ...
              <div className="space-y-4 p-4">
                  ...
                  <div className="grid gap-2 sm:grid-cols-3">
                    {dayMeals.map((meal) => (
                      ...
                      >
                        ...
                        <div className="mt-1 font-semibold text-slate-850">
                          {meal.meal}
                        </div>
                      </div>
                    ))}
                ...
                {dayEvents.length ? (
                  ...
                  <div className="rounded-[1.35rem] bg-white/40 p-4 text-sm text-slate-600">
                    No events yet. A blank schedule: nature’s finest luxury.
                  </div>
                )}
              </div>
            </GlassCard>
  ...
  return (
    <GlassCard className="p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-600">
        This build is ready for PWA setup during deployment: app icon, splash
        color, add-to-home-screen manifest, and offline shell.
      </p>
    </GlassCard>
  );
}
    ...
    <GlassCard className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Shared saving is configured through deployment environment
            variables. Guests will never see the Supabase setup form.
          </p>
        </div>
        <span
          className={`rounded-full px-3 py-1 text-xs font-semibold ${
      ...
      {!supabase && (
        <p className="mt-3 rounded-2xl bg-red-100/70 px-4 py-3 text-sm text-red-800">
          Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in StackBlitz or
          Vercel environment variables, then restart/redeploy.
        </p>
      )}
    </GlassCard>
  );
...
export default function App() {
  ...
  const [selectedGuest, setSelectedGuest] = usePersistentState(
    'mv2026-selected-guest',
    ''
  );
  ...
  const initialGroupPacking = useMemo(
    () => tripData.groupPacking.map((item) => ({ id: item, item, person: '' })),
    []
  );

  const [events, setEvents, eventsSync] = useSharedState(
    sharedKeys.events,
    starterEvents
  );
  const [meals, setMeals, mealsSync] = useSharedState(
    sharedKeys.meals,
    initialMeals
  );
  const [links, setLinks, linksSync] = useSharedState(
    sharedKeys.links,
    starterLinks
  );
  const [groupItems, setGroupItems, groupPackingSync] = useSharedState(
    sharedKeys.groupPacking,
    initialGroupPacking
  );
